...
class PluginInversionesProfile extends Profile {
    ...
    public static function getAllRights() {
        ...
        return array(
            array(
                'itemtype' => 'PluginInversionesProfile',
                'label' => $LANG['plugin_inversiones']['manage_alerts'],
                'field' => 'plugin_inversiones'
            ),
        );
    }
    ...
    public static function changeProfile() {
        ...
        if ($profile->getFromDBByProfile($_SESSION['glpiactiveprofile']['id'])) {
            $_SESSION['glpiactiveprofile']['inversiones_alert'] = $profile->getField('alert');
        } else {
            unset($_SESSION['glpiactiveprofile']['inversiones_alert']);
        }
    }

    public function getTabNameForItem(CommonGLPI $item, $withtemplate = 0) {
        global $LANG;
        return $LANG['plugin_inversiones']['title'];
    }
    ...
    public function showForm($profiles_id, $options = array()) {
        ...
        $profile->displayRightsChoiceMatrix($rights, array(
            ...
            'title' => $LANG['plugin_inversiones']['title'],
        ));
